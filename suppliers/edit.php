...
$page_title = "Edit Supplier - " . htmlspecialchars($supplier['name']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $check_stmt = $conn->prepare("SELECT id FROM suppliers WHERE LOWER(name) = LOWER(?) AND id != ?");
    $check_stmt->bind_param("si", $form_data['name'], $supplier_id);
    $check_stmt->execute();
    ...
    if (!empty($form_data['email'])) {
        $check_stmt = $conn->prepare("SELECT id FROM suppliers WHERE email = ? AND id != ?");
        $check_stmt->bind_param("si", $form_data['email'], $supplier_id);
        $check_stmt->execute();
        if ($check_stmt->get_result()->num_rows > 0) {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE suppliers SET name = ?, contact_person = ?, email = ?, phone = ?, address = ?, city = ?, postal_code = ?, country = ?, is_active = ? WHERE id = ?");
        ...
        if ($stmt->execute()) {
            $_SESSION['success'] = "Supplier '{$form_data['name']}' has been updated successfully!";
            header('Location: index.php');
            exit;
        } else {
...
$po_count_stmt = $conn->prepare("SELECT COUNT(*) FROM purchase_orders WHERE supplier_id = ?");
$po_count_stmt->bind_param("i", $supplier_id);
$po_count_stmt->execute();
$po_count = $po_count_stmt->get_result()->fetch_row()[0];

// Get active purchase orders count
$active_po_stmt = $conn->prepare("SELECT COUNT(*) FROM purchase_orders WHERE supplier_id = ? AND status NOT IN ('cancelled', 'received')");
$active_po_stmt->bind_param("i", $supplier_id);
$active_po_stmt->execute();
$active_po_count = $active_po_stmt->get_result()->fetch_row()[0];
?>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">
                        <i class="fas fa-edit text-blue-600 mr-3"></i>Edit Supplier
                    </h1>
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">
                <i class="fas fa-chart-bar text-green-600 mr-2"></i>Supplier Activity
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600"><?= $po_count ?></div>
                    <div class="text-sm text-gray-600">Total Purchase Orders</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold <?= $active_po_count > 0 ? 'text-orange-600' : 'text-green-600' ?>"><?= $active_po_count ?></div>
                    <div class="text-sm text-gray-600">Active Purchase Orders</div>
                </div>
            </div>
            <?php if ($active_po_count > 0): ?>
            <div class="mt-4 p-3 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded">
                <i class="fas fa-exclamation-triangle mr-2"></i>
                This supplier has active purchase orders. Deactivating will prevent new orders but won't affect existing ones.
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="bg-white rounded-lg shadow mt-6">
            <div class="p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">
                    <i class="fas fa-history text-indigo-600 mr-2"></i>Recent Purchase Orders
                </h3>
                <?php
                $recent_pos = $conn->prepare("SELECT po.po_number, po.status, po.order_date, po.total_amount FROM purchase_orders po WHERE po.supplier_id = ? ORDER BY po.created_at DESC LIMIT 5");
                $recent_pos->bind_param("i", $supplier_id);
                $recent_pos->execute();
                $pos_result = $recent_pos->get_result();
                ?>
